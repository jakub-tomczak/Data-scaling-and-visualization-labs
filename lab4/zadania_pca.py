...
def pca_manual(data, n_comp=None):
    ...
    if n_comp is None:
        n_comp = data.shape[1]
    assert 1 <= n_comp <= data.shape[1], "The number of components cannot be greater than the number of features and must be higher than 0"
    ...
    eig_val, eig_vec = np.linalg.eig(data_cov) # eig_vec are already normalized
    ...
    logging.info('SORTED EIGEN_VECTORS')
    logging.info(K)

    # TODO: 6) Calculate the transformed data.
    Y = data@K # dot product
...
def data_example_0():
    return np.array([[-2, -1, 0, 1, 2], [-2, 0, 1, -1, 2]]).T

def data_example_1():
    return np.array([[3, 1], [2, 2]])
